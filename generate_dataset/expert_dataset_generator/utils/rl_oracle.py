...
class RLOracle(optimization_oracle.AbstractOracle):
    ...
    def generate_new_policies(self, training_parameters):
        ...
        for player in range(len(training_parameters)):
            ...
            # TODO: change best response class to mbrl
    ...
    def __call__(self,
        ...
        # ex = [0, 0]
        num_players = len(training_parameters)
        agent_chosen_dict = training_parameters[0][0]
        total_policies = agent_chosen_dict["total_policies"]
        probabilities_of_playing_policies = agent_chosen_dict["probabilities_of_playing_policies"]
        best_utility = [0 for _ in range(num_players)]

        for agent in range(len(training_parameters)):
            best_utility[agent] = self.evaluate_policy(game, new_policies[agent][0], total_policies, agent, probabilities_of_playing_policies,  **oracle_specific_execution_kwargs)
        print(best_utility)
        on_policy_utility = self.on_policy(game, total_policies, probabilities_of_playing_policies)
        print(on_policy_utility)
        ex = np.array(best_utility) - on_policy_utility
        print(ex)
        return new_policies, ex

    def on_policy(self, game, total_policies, probabilities_of_playing_policies):
        totals = np.zeros(2)
        count = 0
        for _ in range(self._number_policies_sampled):
            policies_selected = optimization_oracle.strategy_sampler_fun(total_policies,
                                                                         probabilities_of_playing_policies)
            new_return = self.sample_episode(game, policies_selected)
